...
# 5. Security Group for CI/CD Tools
resource "aws_security_group" "devops_sg" {
  name        = "devops-ci-cd-sg"
  description = "Allow SSH, HTTP, Jenkins, SonarQube, and App traffic"
  vpc_id      = aws_vpc.devops_vpc.id
  ...
  # Python App / NodePort / Monitoring
  ingress {
    description = "Application & Prometheus/Grafana Ports"
    from_port   = 3000
    to_port     = 9100
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_key_pair" "devops_keypair" {
  key_name   = "devops-e2e-key"
  public_key = tls_private_key.devops_key.public_key_openssh
}

# Save Private Key Locally to terraform/devops-e2e-key.pem
resource "local_file" "devops_private_key" {
  content         = tls_private_key.devops_key.private_key_pem
  filename        = "${path.module}/devops-e2e-key.pem"
  file_permission = "0400"
}

# 6. EC2 Instance for CI/CD Tooling
